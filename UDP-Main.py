...
while True:
    ...
    while first_detected_class is None:
                ...
                if not ret:
                    ...
                    continue
                #D:/Python/TestYoloPoseVer2/dataset/images/train/0_0
                # Save the captured frame as an image
                image_path = "captured_image.jpg"
                cv2.imwrite(image_path, frame)

                # Run detection on the saved image
                results = model.predict(source="captured_image.jpg", show=False, save=False, conf=0.5)
                if len(results) > 0:
                    for r in results:
                        if r.boxes.cls.numel() > 0:  # Check if there are any detections
                            ...
                            # jsonData = json.dumps({"result": first_detected_class})  # Example payload
                            # sock.sendto(jsonData.encode('utf-8'), client_address)
